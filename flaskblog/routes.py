...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('Вам было отправлено электронное письмо с инструкциями по сбросу вашего пароля.', 'info')
        return redirect(url_for('login'))
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Ваш пароль был обновлён! Сейчас вы сможете войти по новому паролю', 'success')
        return redirect(url_for('login'))
